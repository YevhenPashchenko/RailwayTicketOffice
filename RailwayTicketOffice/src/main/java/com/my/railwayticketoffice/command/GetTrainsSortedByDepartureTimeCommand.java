 ...
 * traveling through the station specified in the request in the date specified in the request and sorted them by departure time.
 */
public class GetTrainsSortedByDepartureTimeCommand implements Command {
        ...
        if (checkParametersForCorrectness(request)) {
            try(Connection connection = DBManager.getInstance().getConnection()) {
                ...
                if (trains.size() > 0) {
                    ...
                    request.setAttribute("trainsSortedCommand", "getTrainsSortedByDepartureTime");
                }
