            ...
            if (trainService.check(parameters, session)) {
                ...
                try(Connection connection = DBManager.getInstance().getConnection()) {
                    ...
                    if (isTrainHasCarriageWithThisNumber > 0) {
                        ...
                        int isCarriageExists = trainDAO.checkIfCarriageExists(connection, Integer.parseInt(parameters.get("carriageNumber")), Integer.parseInt(parameters.get("typeId")));
                        if (isCarriageExists == 0) {
                            carriageId = trainDAO.createCarriage(connection, Integer.parseInt(parameters.get("carriageNumber")), Integer.parseInt(parameters.get("typeId")));
                        } else {
                            carriageId = isCarriageExists;
                        }
                    }
                } catch (SQLException e) {
                    logger.info("Failed to connect to database for check if train has carriage with this number");
                    if ("en".equals(session.getAttribute("locale"))) {
                        session.setAttribute("errorMessage", "Failed to connect to database for check if train has carriage with this number");
                    } else {
                        session.setAttribute("errorMessage", "Не вийшло зв'язатися з базою даних, щоб перевірити, чи поїзд вже має вагон з таким номером");
                    }
                    throw new DBException("Failed to connect to database for check if train has carriage with this number");
                }
